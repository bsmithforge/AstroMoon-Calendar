...
- Calendar capture currently uses a 900px-wide off-screen container, `pixelRatio: 2`, `skipFonts: true`, a warm-up capture, and sequential month captures. Tailwind viewport breakpoints still depend on the actual browser viewport, so a fixed capture width alone does not guarantee identical output from a phone. Check fonts, SVGs, and layout in files generated from both mobile and desktop.
...
- Design for touch and narrow screens: readable text, comfortably tappable controls, wrapping labels, and no unintended page-level horizontal scrolling or clipped actions. Check the global `button { white-space: nowrap; }` rule when diagnosing overflow.
...
- `ExportModal` returns before declaring hooks when closed. Check its hook ordering and state lifecycle when working on modal opening, closing, or range synchronization.
